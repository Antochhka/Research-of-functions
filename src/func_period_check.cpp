...
std::string findFunctionPeriod(vector<vector<double>> &output_arr, unsigned int &i_out_arr) {
    ...
    const vector<double> periods = {M_PI / 2, M_PI, 3 * M_PI / 2, 2 * M_PI}; // Возможные значения для периода
    double y_left = calculator(output_arr, i_out_arr, left_x);

    // Создаем временную переменную для хранения x, чтобы избежать проблем с ссылкой
    for (double period : periods) {
        double current_x = left_x + period;
        double y_at_period = calculator(output_arr, i_out_arr, current_x);
        // Проверяем, совпадает ли значение с y на границе LEFT_BORDER с учетом погрешности
